...
io.on('connection', (socket) =>{//instantiate and establish the socket so that we can connect to the client
    ...
    socket.on('coord', (userAdding) => {//user is adding an image
        ...
        io.emit('update', `client: ${socket.id} posted in location (X:${userAdding.x}, Y:${userAdding.y}) reload to see it!`);//can update live all I am doing is simply sending an image
        //notify all clients
